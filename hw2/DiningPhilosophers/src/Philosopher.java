...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		int time_to_table = (int) (Math.random() * 10);
		try {
			sleep(time_to_table*1000);
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		table.PutPlate_GUI(id);

		// Barrier - to wait other philosophers
		barriers[id].release();
		barriers[id].release();
		barriers[id].release();
		barriers[id].release();
		try {
			barriers[(id + 1) % 5].acquire();
			barriers[(id + 2) % 5].acquire();
			barriers[(id + 3) % 5].acquire();
			barriers[(id + 4) % 5].acquire();
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		...
		while (true) {
			// -- think
			state[id] = THINKING;
			int think_time = (int) (Math.random() * 10);
			try {
			...
			try {
				...
				state[id] = THINKING;
				if (state[(id + 1) % N] == HUNGRY && state[(id + 2) % N] != EATING && state[(id) % N] != EATING) {
					state[(id + 1) % N] = EATING;
					sem[(id + 1) % N].release();
				}
				if (state[(id + N -1) % N] == HUNGRY && state[(id) % N] != EATING && state[(id + N -2) % N] != EATING) {
					state[(id + N -1) % N] = EATING;
					sem[(id + N -1) % N].release();
				}
				mutex.release();
